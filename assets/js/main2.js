async function loadContent(){
    ...
    await fetch('./index.json').then(response => {
        ...
        for(var i=0;i<data.commands.length;i++){
            titleShow += `<a type="button" id="`+ data.commands[i].name +`" onClick="fetchTitle(this.id)"><li class="nav-item my-1 p-2 highlight">`+ data.commands[i].name +`</li></a >`;
        }
...
function showdocumetation(title){
    ...
    fetch("doc_files/pages/common/" + title + ".md")
